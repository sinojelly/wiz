...
CString CyberArticleGetConvertFlashToHTMLTemplateFileName(LPCTSTR lpszCyberArticleAppPath)
{
	...
	return strPath + _T("files\\ImportFlash.htm");
}
...
CString CyberArticleGetConvertImageToHTMLTemplateFileName(LPCTSTR lpszCyberArticleAppPath)
{
	CString strPath(lpszCyberArticleAppPath);
	WizPathAddBackslash(strPath);
	return strPath + _T("files\\ImportImage.htm");
}
...
CString CyberArticleGetConvertTextToHTMLTemplateFileName(LPCTSTR lpszCyberArticleAppPath)
{
	CString strPath(lpszCyberArticleAppPath);
	WizPathAddBackslash(strPath);
	return strPath + _T("files\\ImportText.htm");
}
...
const LPCTSTR g_lpszSectionOfURLAndFile = _T("URLAndFile");


BOOL CWizHTMLURLAndFileDataArray::LoadFromIni(LPCTSTR lpszText)
{
	clear();
	//
	CWizIniFileEx ini;
	ini.LoadFromText(lpszText);
	//
	int nCount = ini.GetIntDef(g_lpszSectionOfURLAndFile, _T("Count"), 0);
	for (int i = 0; i < nCount; i++)
	{
		CString strIndex = WizIntToStr(i);
		//
		WIZHTMLURLANDFILEDATA data;
		data.strURL = ini.GetStringDef(g_lpszSectionOfURLAndFile, strIndex + _T("_URL"));
		data.strFileName = ini.GetStringDef(g_lpszSectionOfURLAndFile, strIndex + _T("_FileName"));
		//
		push_back(data);
	}
	//
...
BOOL CWizHTMLURLAndFileDataArray::SaveToIni(CString& strText) const
{
	CWizStdStringArray arrayText;
	arrayText.push_back(WizFormatString1(_T("[%1]"), g_lpszSectionOfURLAndFile));
	arrayText.push_back(_T("Count=") + WizIntToStr(int(size())));
	for (size_t i = 0; i < size(); i++)
	{
		CString strIndex = WizIntToStr(int(i));
		arrayText.push_back(strIndex + _T("_URL=") + at(i).strURL);
		arrayText.push_back(strIndex + _T("_FileName=") + at(i).strFileName);
	}
	//
...
const LPCTSTR g_lpszSectionOfFrames = _T("Frames");

BOOL CWizFrameArray::LoadFromIni(LPCTSTR lpszText)
{
	clear();
	//
	CWizIniFileEx ini;
	ini.LoadFromText(lpszText);
	//
	int nCount = ini.GetIntDef(g_lpszSectionOfFrames, _T("Count"), 0);
	for (int i = 0; i < nCount; i++)
	{
		CString strIndex = WizIntToStr(i);
		//
		WIZHTMLFRAMEDATA data;
		data.strURL = ini.GetStringDef(g_lpszSectionOfFrames, strIndex + _T("_FrameURL"));
		data.strName = ini.GetStringDef(g_lpszSectionOfFrames, strIndex + _T("_FrameName"));
		data.strID = ini.GetStringDef(g_lpszSectionOfFrames, strIndex + _T("_FrameID"));
		data.strExtName = ini.GetStringDef(g_lpszSectionOfFrames, strIndex + _T("_FrameExtName"));
		data.strFileName = ini.GetStringDef(g_lpszSectionOfFrames, strIndex + _T("_FrameFileName"));
		data.strIncludeScriptFileName = ini.GetStringDef(g_lpszSectionOfFrames, strIndex + _T("_FrameInclueScriptFileName"));
		data.strNoScriptFileName = ini.GetStringDef(g_lpszSectionOfFrames, strIndex + _T("_FrameNoScriptFileName"));
		//
		push_back(data);
	}
	//
...
BOOL CWizFrameArray::SaveToIni(CString& strText) const
{
	CWizStdStringArray arrayText;
	arrayText.push_back(WizFormatString1(_T("[%1]"), g_lpszSectionOfFrames));
	arrayText.push_back(_T("Count=") + WizIntToStr(int(size())));
	for (size_t i = 0; i < size(); i++)
	{
		const WIZHTMLFRAMEDATA& data = at(i);
		//
		CString strIndex = WizIntToStr(int(i));
		arrayText.push_back(WizFormatString2(_T("%1_FrameURL=%2"), strIndex, data.strURL));
		arrayText.push_back(WizFormatString2(_T("%1_FrameName=%2"), strIndex, data.strName));
		arrayText.push_back(WizFormatString2(_T("%1_FrameID=%2"), strIndex, data.strID));
		arrayText.push_back(WizFormatString2(_T("%1_FrameExtName=%2"), strIndex, data.strExtName));
		arrayText.push_back(WizFormatString2(_T("%1_FrameFileName=%2"), strIndex, data.strFileName));
		arrayText.push_back(WizFormatString2(_T("%1_FrameInclueScriptFileName=%2"), strIndex, data.strIncludeScriptFileName));
		arrayText.push_back(WizFormatString2(_T("%1_FrameNoScriptFileName=%2"), strIndex, data.strNoScriptFileName));
	}
	//
	::WizStringArrayToText(arrayText, strText, _T("\n"));
	return TRUE;
