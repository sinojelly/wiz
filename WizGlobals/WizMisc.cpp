...
CString WizCmdLineToFileNameEx(LPCTSTR lpszCmdLine)
{
	CString strRet;
	//
	if (!lpszCmdLine)
	...
	if (!*lpszCmdLine)
		return strRet;
	//
	if (_tcslen(lpszCmdLine) >= 1024)
		return strRet;
	//
	TCHAR szCmdLine[1024] = {0};
	_tcscpy_s(szCmdLine, 1024, lpszCmdLine);
	//
	if (!WizCmdLineToFileName(szCmdLine))
		return strRet;
	//
	return CString(szCmdLine);
}
...
{
	CString strFileName = ::WizGetAppPath() + WizGetInnerProductName() + _T(".ini");
	return strFileName;
}
...
static CString g_strSecuteText;
void WizSetSecuteString(LPCTSTR lpszText)
{
	g_strSecuteText = lpszText;
}
CString WizGetSecuteSettings(LPCTSTR lpszSection, LPCTSTR lpszValueName, BOOL bAllowEmpty)
{
	...
	if (str.IsEmpty())
	{
		...
		{
#ifdef _DEBUG
			MessageBox(NULL, CString(_T("[")) + lpszSection + _T("]\n") + lpszValueName + _T("="), NULL, 0);
#endif
			ATLASSERT(FALSE);
			exit(-1);
		}
...
CString WizGetInnerProductName()
{
	static CString strProductName;
	if (!strProductName.IsEmpty())
		return strProductName;
	//
	strProductName = WizGetSecuteSettings(_T("Common"), _T("InnerProductName"), TRUE);
	if (strProductName.IsEmpty())
	{
		strProductName = WizGetProductName();
	}
	//
	return strProductName;
...
CString WizTranslationsGetLCIDName(LCID lcid)
{
	if (GetACP() == 936)
	{
		if (lcid == 2052)
		{
			return CString(_T("简体中文"));
		}
		else if (lcid == 1028)
		{
			return CString(_T("繁体中文"));
		}
	}
	//
	TCHAR szName[1024] = {0};
...
BOOL WizTranslationsEnumSupportedLanguages(std::vector<LCID>& arrayLCIDs)
{
	...
	//
	arrayLCIDs.push_back(1033);
	//
	return TRUE;
}

BOOL WizTranslationsEnumSupportedLanguages(std::map<LCID, CString>& arrayLanguage)
{
	std::vector<LCID> arrayLCIDs;
	WizTranslationsEnumSupportedLanguages(arrayLCIDs);
	//
	for (std::vector<LCID>::const_iterator it = arrayLCIDs.begin();
		it != arrayLCIDs.end();
		it++)
	{
		arrayLanguage[*it] = WizTranslationsGetLCIDName(*it);
	}
	//
	return TRUE;
}
...
BOOL CWizTranslationsStringTable::TranslateString(LPCTSTR lpszSrc, CString& strRet)
{
	strRet = lpszSrc;
	//
	if (0 == strRet.CompareNoCase(_T("{p}")))
	{
		strRet = WizGetProductName();
		return TRUE;
	}
	//
...
BOOL CWizURL::Combine(const CWizURL& urlRelative)
{
	...
	if (FAILED(UrlCombine(m_strURL, strRelativeURL, szCombined, &dwLen, 0)))
		return FALSE;
	m_strURL = CWizURL(szCombined, 0, FALSE);
	return TRUE;
}
...
CString CWizXMLNode::GetFirstChildNodeText(LPCTSTR lpszDef /* = NULL */)
{
	CWizXMLNode nodeChild;
	if (!GetFirstChildNode(nodeChild))
		return CString(lpszDef);
	//
	return nodeChild.GetText(lpszDef);
}
