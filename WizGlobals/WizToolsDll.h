...
class CWizToolsParseHtmlResourceEvents : public IWizToolsParseHtmlResourceEvents
{
public:
	CWizToolsParseHtmlResourceEvents(CWizHTMLResourceFileDataArray& arrayResource)
		: m_arrayResource(arrayResource)
	{
	}
	CWizHTMLResourceFileDataArray& m_arrayResource;
public:
	virtual HRESULT __stdcall OnHtmlResource(LPCWSTR lpszURL, LPCWSTR lpszFileName, IStream* pStream)
	{
		WIZHTMLRESOURCEFILEDATA data;
		data.strURL = lpszURL;
		data.strFileName = lpszFileName;
		data.spStream = pStream;
		m_arrayResource.push_back(data);
		//
		return S_OK;
	}
};
...
public:
	CWizToolsDll(LPCWSTR lpszLogFileName, LPCTSTR lpszSettingsFileName, LPCTSTR lpszWizToolsPath = NULL)
	{
		...
		m_pfunPublishHtml = NULL;
		m_pfunSetSecuteText = NULL;
		m_pfunHtmlResource2HtmlResource = NULL;
		//
		...
		if (m_hModule)
		{
			...
			m_pfunPublishHtml = (Type_WizToolsPublishHtml *)GetProcAddress(m_hModule, "WizToolsPublishHtml");
			m_pfunSetSecuteText = (Type_WizToolsSetSecuteText*)GetProcAddress(m_hModule, "WizToolsSetSecuteText");
			m_pfunHtmlResource2HtmlResource = (Type_WizToolsHtmlResource2HtmlResource*)GetProcAddress(m_hModule, "WizToolsHtmlResource2HtmlResource");
			
			//
#ifdef _WIZ_SECUTE_TEXT
			SetSecuteText(_WIZ_SECUTE_TEXT);
#endif
			//
			if (lpszLogFileName)
			{
...
public:
	...
	typedef int WINAPI Type_WizToolsPublishHtml(HWND hwnd, HINSTANCE hinst, LPCWSTR lpszCmdLine, int nCmdShow);
	typedef STDMETHODIMP Type_WizToolsHtmlResource2HtmlResource(LPCWSTR lpszResourceText, UINT nHtmlSaverFlags, LPCWSTR lpszExtParams, IWizToolsParseHtmlResourceEvents* pEvents);
...
protected:
	...
	Type_WizToolsPublishHtml* m_pfunPublishHtml;
	Type_WizToolsSetSecuteText* m_pfunSetSecuteText;
	Type_WizToolsHtmlResource2HtmlResource* m_pfunHtmlResource2HtmlResource;
...
public:
	...
	BOOL Html2Text(LPCWSTR lpszHtml, UINT nFlags, CString& strText)
	{
		CComBSTR bstrText;
		if (FAILED(Html2Text(lpszHtml, nFlags, &bstrText)))
			return FALSE;
		//
		strText = CString(bstrText);
		//
	...
	BOOL Html2TextIncludeTitle(LPCWSTR lpszHtml, CString& strText)
	{
		return Html2Text(lpszHtml, 0x01, strText);
	}
...
#define s_nIVLen	16
	...
	//
	BOOL PublishHtml(LPCTSTR lpszCommandFileName)
	{
		if (!m_pfunPublishHtml)
			return FALSE;
		if (!lpszCommandFileName || !*lpszCommandFileName)
			return FALSE;
		//
		CString strCommandLine = CString(_T("/CommandFileName=")) + lpszCommandFileName;
		//
		return 0 == m_pfunPublishHtml(NULL, NULL, CStringW(strCommandLine), SW_SHOW);
	}
	void SetSecuteText(LPCWSTR lpszText)
	{
		if (!m_pfunSetSecuteText)
			return;
		//
		m_pfunSetSecuteText(lpszText);
	}
	//
	HRESULT HtmlResource2HtmlResource(LPCWSTR lpszResourceText, UINT nHtmlSaverFlags, CWizHTMLResourceFileDataArray& arrayResource)
	{
		if (!m_pfunHtmlResource2HtmlResource)
			return E_FAIL;
		//
		CWizToolsParseHtmlResourceEvents events(arrayResource);
		//
		HRESULT hr = m_pfunHtmlResource2HtmlResource(lpszResourceText, nHtmlSaverFlags, NULL, &events);
		if (FAILED(hr))
			return hr;
		//
		return hr;
