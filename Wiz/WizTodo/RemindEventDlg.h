...
private:
	...
	{
		CRect rcReminder;
		GetWindowRect(&rcReminder);
		//
		int width = ::GetSystemMetrics(SM_CXSCREEN);
		int height = ::GetSystemMetrics(SM_CYSCREEN);
		//
		int x = width - rcReminder.Width() - 40;
		int y = height - rcReminder.Height() - 40;
		//
		if (x <= 0 || y <= 0)
		{
			x = 0;
		...
		//
		rcReminder.MoveToXY(x, y);
		MoveWindow(rcReminder, FALSE);
	}
public:
	...
	LRESULT OnBnClickedButtonComplete(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
	{
		...
		if (WizKMGetPrivateBool(_T("Event"), _T("CompleteTodoWhileEventCompleted"), TRUE))
		{
			...
			//
			WizGetMainDlg()->PostMessage(UM_REFRESH_TODOLISTS, 0, 0);
		}
		//
		return 0;
	}
