...
inline CString WizKMGetProductExeFileTitle()
{
	return WizGetInnerProductName();
}

inline CString WizKMGetProductExeFileNameNoPath()
{
	return WizKMGetProductExeFileTitle() + _T(".exe");
}

inline CString WizKMGetProductExeFileName()
{
	return WizGetAppPath() + WizKMGetProductExeFileNameNoPath();
}

inline CString WizKMGetExplorerExeFileName()
{
...
{
	static BOOL b = PathFileExists(WizKMGetExplorerExeFileName());
	return b;
}
...
{
	CString strName = WizGetInnerProductName() + _T(".ini");
	//
...
public:
	...
	{
		return CWizToolsDll::ShowBubbleWindow(WizGetProductName(), lpszSubTitle, lpszIconFileName, lpszText, lpszButtonText, nDelaySeconds, lpszExtOptions);
	}
...
{
	return WizKMGetToolsDll().ShowBubbleWindowDefaultIcon(lpszSubTitle, lpszText, lpszCustomButtonText, nDelay, lpszExtOptions);
}
...
{
	return WizKMGetToolsDll().ShowBubbleWindow2(lpszTitle, lpszLinkText, lpszLinkURL, crBackground, nDelay);
}
...
inline BOOL WizKMDownloadAD(LPCTSTR lpszRelatedURL)
{
	...
	return WizURLADDownloadToXmlFile(WizGetInnerProductName(), lpszRelatedURL, WizKMGetSettingsFileName(), WIZKM_SETTINGS_ROOT_NODE_NAME, strVersion);
}
...
inline BOOL WizKMDisplayOneAD(LPCTSTR lpszCategory)
{
	...
	{
		strCategory = WizGetInnerProductName();
	}
	//
...
inline const WIZCOMOBJECTDATA* WizKMGetComObjectDataArray()
{
	static const WIZCOMOBJECTDATA data[] =
	{
		...
		{_T("WizTools.WizCommonTools"),						_T("{4511852F-A672-426F-8860-B0B688D2A467}"),	_T("WizTools.dll")},
		{_T("WizTools.WizDbxFile"),							_T("{80DA3F56-12C9-4588-A0C1-A67C51EED025}"),	_T("WizTools.dll")},
...
inline BOOL WizKMHtmlContentRulesUploadAll(long* pnCount = NULL)
{
	...
	//
	HRESULT hr = WizKMGetToolsDll().HtmlContentRulesUploadAll(strURL, pnCount);
	if (FAILED(hr))
		return FALSE;
	//
	return TRUE;
...
inline BOOL WizKMHtmlContentRulesDownloadAll(long* pnCount = NULL)
{
	...
	//
	HRESULT hr = WizKMGetToolsDll().HtmlContentRulesDownloadAll(strURL, pnCount);
	if (FAILED(hr))
		return FALSE;
	//
	return TRUE;
